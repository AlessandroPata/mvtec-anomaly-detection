...
Le metriche di localizzazione e di hardening (sezioni 7.5, 8.9, 9.1) provengono da artefatti dedicati, generati dal vivo dai modelli di produzione e versionati in `production_models/`: `pixel_metrics.json` (pixel-AUROC, pixel-AP, AUPRO), `honest_calibration.json` (oracle vs. held-out vs. p99) ed `ensemble_experiment.json` (studio di fusione GAN↔PatchCore), prodotti rispettivamente da `pixel_metrics.py`, `honest_calibration.py` ed `ensemble_experiment.py`.
