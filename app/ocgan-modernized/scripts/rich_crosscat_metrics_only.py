...
def parse_args():
    ...
    p.add_argument("--out-dir", default="./grid_runs/rich_crosscat_metrics_only")
...
def build_specs(args) -> List[Spec]:
    ...
    teacher_pool = teacher_states()
    memory_pool = memory_states()
    fusion_pool = learned_fusion_states()
    oc_pool = one_class_states()

    for category in args.categories:
        for seed in args.seeds:
            for t_name, t_over in teacher_pool:
                for m_name, m_over in memory_pool:
                    for lf_name, lf_over in fusion_pool:
                        for oc_name, oc_over in oc_pool:
...
def main():
        ...
        "specs": [
            {
                ...
                "teacher": s.teacher,
                "memory": s.memory,
                "learned_fusion": s.learned_fusion,
                "one_class_score": s.one_class_score,
    ...
    current_spec_names = {s.name for s in specs}
    all_done_names = completed_names_for_resume(existing_by_name, args.retry_failed)
    done_names = all_done_names & current_spec_names
    ...
    with ThreadPoolExecutor(max_workers=args.max_parallel) as ex:
        ...
        with tqdm(total=len(specs), initial=len(done_names), desc="grid") as pbar:
