#!/usr/bin/env bash
# v3b: retrain delle categorie affette da NaN con unfreeze_from=none.
# Root cause: backbone fine-tuning con mixed-anomaly batches crea drift oscillatorio
# nelle BN running stats. Fix: backbone frozen, UNet skip connections mantenute.
# Categorie target: capsule, cable, screw, pill, metal_nut + tile + transistor
# (tutte con non_finite events nei log v3 full).
set -euo pipefail

cd /notebooks/storage_project_outputs_datasets/project/ocgan-modernized

LOG_DIR="logs/v3b_fix_noUnfreeze"
STATE_DIR="logs/v3b_fix_noUnfreeze_state"
CATEGORIES=(capsule cable screw pill metal_nut tile transistor)
...
run_one() {
  local category="$1"
  local seed="$2"
  local run_name="${category}_v3b_s${seed}"
...
echo "=== v3b fix noUnfreeze: $(wc -l < "${TMP_TASKS}") runs, parallel=${PARALLEL_JOBS} ==="
